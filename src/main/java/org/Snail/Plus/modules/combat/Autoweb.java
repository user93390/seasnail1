...
public class Autoweb extends Module {
    ...
    public void OnTick(TickEvent.Pre event) {
        ...
        if ((SyncDelay - lastWebPlaceTime) < delay.get() * 1000) return;
        lastWebPlaceTime = currentTime;
        ...
        for (CardinalDirection dir : CardinalDirection.values()) {
            if (this.strictDirection.get() && dir.toDirection() != Objects.requireNonNull(mc.player).getHorizontalFacing()
    ...
    if(mc.player != null && mc.world != null && target != null) {
        BestWeb(target, InvUtils.findInHotbar(Items.COBWEB));
        }
    }
    @EventHandler
    ...
    public void BestWeb(PlayerEntity target, FindItemResult item) {
            if (smart.get() && Double.get() && CombatUtils.isSurrounded(target)) {
                InvUtils.swap(item.slot(), false);
                Objects.requireNonNull(mc.interactionManager).interactBlock(mc.player, Hand.MAIN_HAND, new BlockHitResult(new Vec3d(targetPos.getX() + 0.5, targetPos.getY(), targetPos.getZ()), Direction.UP, targetPos, false));
                currentPos = targetPos;
                Objects.requireNonNull(mc.interactionManager).interactBlock(mc.player, Hand.MAIN_HAND, new BlockHitResult(new Vec3d(DoubleWeb.getX() + 0.5, DoubleWeb.getY() , DoubleWeb.getZ()), Direction.UP, DoubleWeb, false));
